...
The command uses the packaged AppVerk workflow, routes the final commit through the controlled runtime, and blocks direct `git commit` and `git push` from the bash tool.

## Available Packages

| Package | Version | Description |
|---------|---------|-------------|
| [`@appverk/opencode-commit`](docs/plugins/commit.md) | `0.1.0` | Controlled OpenCode commit workflow with automatic `/commit` registration, policy enforcement, and bash-level blocking for direct `git commit` and `git push`. |
